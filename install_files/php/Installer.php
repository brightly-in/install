...
class Installer
{
    ...
    protected function onCheckRequirement()
    {
        ...
        switch ($checkCode) {
            ...
            case 'writePermission':
                ...
                break;
            case 'phpVersion':
                $result = PHP_VERSION_ID >= BRIGHTLY_MINIMUM_PHP_VERSION_ID;
                break;
            case 'pdoLibrary':
    ...
    protected function onInstallStep()
    {
        ...
        switch ($installStep) {
            ...
            case 'extractCore':
                ...
                $this->moveHtaccess(null, 'brightly');
            ...
            case 'finishInstall':
                $this->setCoreBuild();
                $this->moveHtaccess(null, 'installer');
                $this->moveHtaccess('brightly', null);
                $this->cleanUp();
                break;
        }
    ...
    protected function prepareServerRequest($uri, $params = array())
    {
        ...
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, BRIGHTLY_GATEWAY.'/'.$uri);
        ...
        if (defined('BRIGHTLY_GATEWAY_AUTH')) {
            curl_setopt($curl, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
            curl_setopt($curl, CURLOPT_USERPWD, BRIGHTLY_GATEWAY_AUTH);
        }
